...
# Tabella degli eventi anomali (salviamo solo quelli ritenuti di rilievo)
cursor.execute("""
...
# Tabella di configurazione con parametri aggiornabili, incluso il flag per le notifiche
cursor.execute("""
...
# Inserimento della configurazione di default se non esiste
...
if cursor.fetchone()[0] == 0:
    cursor.execute("""
        ...
        VALUES (2.0, 70.0, 0.1, 'triple', 300, 300, '', '', 1)
    """)
    conn.commit()
...
def get_config():
    ...
    cursor.execute("""
        SELECT accel_threshold, gyro_threshold, sample_interval, visualization_mode, chart_buffer, data_retention
    ...
    return {"accel_threshold": 2.0, "gyro_threshold": 70.0, "sample_interval": 0.1, "visualization_mode": "triple", "chart_buffer": 300, "data_retention":300}

def read_mpu():
    """
    Legge i dati dal MPU-6050 e li converte in valori fisici.
